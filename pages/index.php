...
<div id='container'>
	<div id='menu'>
		<ul class='top_menu'>
			<a class='visit' href='index.php'><li>Home</li></a> |
		...
		<table id='content-spliter' cellspacing='20px'>
			<tr valign='top'>
				<td width='70%'>
					...
					<div id='welcome'>
						<?php 
							echo $hasil_master['welcome']; 
						?>
					</div><br/>


					<h3 class='tag_title'><img src='../img/news.gif'/>NEWS & ARTIKEL</h3>
					<hr/><br/>
					<div class='news'>
						<?php
							//ini untuk ambil data di tabel posting kategori news (tbl_posting)
							$qry_posting_news = "select * from tbl_posting where category='news' or category='artikel' order by id_post desc limit 0,2;";
							...
							for($i=0; $hasil = mysqli_fetch_array($qry_exc_news,MYSQLI_ASSOC); $i++){
								...
								echo "<a href='vnews.php?id=".$hasil['id_post']."'>";
								echo "<p>".$hasil['jdl_post']."</br><i>".$hasil['source']."</i></p>";
								echo "</a>";
								echo "</div>";
								echo "<div class='content-news'>";
								echo "<p>".substr($hasil['isi'],0,400)."<br/><a href='vnews.php?id=".$hasil['id_post']."' class='more'>&rArr;Read more</a></p>";
								echo "</div></br>";
							}						
						?>
					</div>
					<h3 class='tag_title'><p align='right'><a href='#' class='more'>See other News..</a></p></h3>					
					<hr/>
					<br/>
					
					<h3 class='tag_title'><img src='../img/rule.gif'/>TAX UPDATES...</h3>
					<hr/><br/>
					<div class='news'>
						<?php
							//ini untuk ambil data di tabel posting kategori tax update (tbl_posting)
							$qry_posting_news = "select * from tbl_posting where category='taxupd'order by id_post desc limit 0,2;";
							$qry_exc_news = mysqli_query($conn,$qry_posting_news);
							for($i=0; $hasil = mysqli_fetch_array($qry_exc_news,MYSQLI_ASSOC); $i++){
								echo "<div class='title_news'>";
								echo "<a href='#'>";
								...
								echo "</div>";
								echo "<div class='content-news'>";
								echo "<p>".substr($hasil['isi'],0,600)."<br/><a href='#' class='more'>&rArr;Read more</a></p>";
								echo "</div></br>";
							}						
						?>
					</div>
					<h3 class='tag_title'><p align='right'><a href='#' class='more'>See Other Updates..</a></p></h3>					
					<hr/>
				</td>
				<td width='30%'>
					<div>
